...
function openPlaylist(id, opts = {}) {
  ...
  if (pushHistory) {
    history.pushState({ page: 'playlist-view', playlistId: id }, '', location.href);
  }
}
...
function renderPlaylistView() {
  ...
  resolved.forEach(({ ref, song }, index) => {
    ...
    removeBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      removeSongFromPlaylist(pl.id, ref.sourceKey, ref.songId);
      renderPlaylistView();
...
function promptRenamePlaylist(id) {
  const pl = getPlaylist(id);
  if (!pl) return;
  const form = playlistNameForm(pl.name, (name) => {
    renamePlaylist(id, name);
    closeModal();
    showToast(t('toastPlaylistRenamed'));
    if (state.activePlaylistId === id) renderPlaylistView();
    if (state.currentPage === 'playlists') renderPlaylistsList();
  });
  openModal(t('renamePlaylistTitle'), form);
}
